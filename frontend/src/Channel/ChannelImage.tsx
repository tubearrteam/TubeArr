...
import { CoverType, Image } from './Channel';

const EMPTY_IMAGES: Image[] = [];

function findImage(images: Image[] | undefined, coverType: CoverType) {
  return (images ?? EMPTY_IMAGES).find((image) => image.coverType === coverType);
}
...
function ChannelImage({
  ...
  const imgProps = {
    className,
    style,
    src: lazy ? url : (isLoaded ? url : placeholder),

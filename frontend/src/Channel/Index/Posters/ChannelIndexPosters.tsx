...
import ChannelIndexPoster from 'Channel/Index/Posters/ChannelIndexPoster';
...
interface ChannelIndexPostersProps {
  items: Channel[];
  sortKey: string;
...
const channelIndexSelector = createSelector(
  (state: AppState) => state.channelIndex.posterOptions,
  (posterOptions) => {
    return {
      posterOptions,
    };
...
export default function ChannelIndexPosters(props: ChannelIndexPostersProps) {
  const {
    scrollerRef,
    items,
    sortKey,
  ...
  const { posterOptions } = useSelector(channelIndexSelector);
  ...
  const posterHeight = posterWidth;

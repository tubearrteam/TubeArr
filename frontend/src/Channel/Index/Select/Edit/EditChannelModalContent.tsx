...
import MoveChannelModal from 'Channel/MoveChannel/MoveChannelModal';
...
function EditChannelModalContent(props: EditChannelModalContentProps) {
  const { channelIds, onSavePress, onModalClose } = props;
  ...
  const save = useCallback(
    (moveFiles: boolean) => {
      ...
      if (channelType !== NO_CHANGE) {
        hasChanges = true;
        payload.channelType = channelType;
      }

      if (playlistFolder !== NO_CHANGE) {
        hasChanges = true;
    ...
    ({ name, value }: { name: string; value: string }) => {
      switch (name) {
        ...
        case 'rootFolderPath':
          ...
          break;
        default:
          console.warn('EditChannelModalContent Unknown Input');
      }
    },
    [setMonitored]
  );
  ...
  const onDoNotMoveChannelPress = useCallback(() => {
    setIsConfirmMoveModalOpen(false);
    save(false);
  }, [setIsConfirmMoveModalOpen, save]);

  const onMoveChannelPress = useCallback(() => {
    setIsConfirmMoveModalOpen(false);
    save(true);
  }, [setIsConfirmMoveModalOpen, save]);
  ...
  return (
    <ModalContent onModalClose={onModalClose}>
      <ModalHeader>{translate('EditSelectedChannel')}</ModalHeader>
      ...
      <ModalBody>
        ...
        <FormGroup>
          <FormLabel>{translate('ChannelType')}</FormLabel>
      ...
      <MoveChannelModal
        ...
        onSavePress={onDoNotMoveChannelPress}
        onMoveChannelPress={onMoveChannelPress}
      />
    </ModalContent>
  );
}

...
interface ChannelTag extends TagBase {
  id: number;
  name: string;
}
...
function createChannelTagsSelector(tags: number[]) {
  return createSelector(createTagsSelector(), (tagList) => {
    ...
    return {
      tags: tags.reduce((acc: ChannelTag[], tag) => {
...
export default function ChannelTagInput({
  name,
  value,
  onChange,
  ...
  const { tags, tagList, allTags } = useSelector(
    createChannelTagsSelector(arrayValue)
  );

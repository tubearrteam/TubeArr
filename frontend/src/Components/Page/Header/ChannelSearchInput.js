...
class ChannelSearchInput extends Component {
  ...
  componentDidMount() {
    this.props.bindShortcut(shortcuts.CHANNEL_SEARCH_INPUT.key, this.focusInput);
  }
  ...
  renderSuggestion(item, { query }) {
    if (item.type === ADD_NEW_TYPE) {
      return (
        <div className={styles.addNewChannelSuggestion}>
  ...
  goToChannel(item) {
    this.setState({ value: '' });
    this.props.onGoToChannel(item.item.titleSlug);
  }
  ...
  onKeyDown = (event) => {
    ...
    if (!suggestions.length || highlightedSectionIndex) {
      this.props.onGoToAddNewChannel(value);
    ...
    // If a suggestion is not selected go to the first channel,
    // otherwise go to the selected channel.

    if (highlightedSuggestionIndex == null) {
      this.goToChannel(suggestions[0]);
    } else {
      this.goToChannel(suggestions[highlightedSuggestionIndex]);
    }
  ...
  onSuggestionSelected = (event, { suggestion }) => {
    if (suggestion.type === ADD_NEW_TYPE) {
      this.props.onGoToAddNewChannel(this.state.value);
    } else {
      this.goToChannel(suggestion);
    }
  };
  ...
  render() {
    ...
    if (suggestions.length || loading) {
      suggestionGroups.push({
        title: translate('ExistingChannel'),
        loading,
        suggestions
      });
    }
    ...
    const theme = {
      ...
      suggestionsContainer: styles.channelSuggestionsContainer,
...
ChannelSearchInput.propTypes = {
  ...
  onGoToAddNewChannel: PropTypes.func.isRequired,
  bindShortcut: PropTypes.func.isRequired
};

export default keyboardShortcuts(ChannelSearchInput);

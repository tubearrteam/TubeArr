...
function isClearableDbOpsQueueItem(item: Command) {
  return isActiveDbOpsQueueItem(item);
}

export default function DbOpsQueuePage() {
  ...
  const title = translate('DatabaseOperationsQueue');
  const [isClearConfirmOpen, setIsClearConfirmOpen] = useState(false);
  const [isClearing, setIsClearing] = useState(false);

  const clearableCommandIds = useMemo(() => {
    return commandItems.filter(isClearableDbOpsQueueItem).map((item) => item.id);
  }, [commandItems]);
  ...
  return (
    <PageContent title={title}>
      ...
      <ConfirmModal
        ...
        message={translate('ClearDbOpsQueueMessage')}
      ...
      <PageContentBody>
        <QueuedTasks
          ...
          filterPredicate={isActiveDbOpsQueueItem}
          columns={columns}
          rowVariant="metadata"
        />
      </PageContentBody>

...
function createMapStateToProps() {
  return createSelector(
    ...
    (organizePreview, naming, channel) => {
      const props = { ...organizePreview };
      props.isFetching = organizePreview.isFetching || naming.isFetching;
      props.isPopulated = organizePreview.isPopulated && naming.isPopulated;
      props.error = organizePreview.error || naming.error;
      props.renameVideos = naming.item.renameVideos;
      props.videoFormat = naming.item[`${channel.channelType}VideoFormat`];
      props.path = channel.path;
...
class OrganizePreviewModalContentConnector extends Component {
  ...
  onOrganizePress = (files, forceCollisions) => {
    this.props.executeCommand({
      name: commandNames.RENAME_FILES,
      channelId: this.props.channelId,
      files,
      force: !!forceCollisions
    });

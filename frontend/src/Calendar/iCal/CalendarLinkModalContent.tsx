...
function CalendarLinkModalContent({
  ...
  const { iCalHttpUrl, iCalWebCalUrl } = useMemo(() => {
    let icalUrl = `${window.location.host}${window.TubeArr.urlBase}/feed/v3/calendar/TubeArr.ics?`;
    ...
    icalUrl += `apikey=${encodeURIComponent(window.TubeArr.apiKey)}`;
        ...
        <Form>
          <FormGroup>
            ...
            <FormInputGroup
              ...
              name="unmonitored"
              value={unmonitored}
              helpText={translate('ICalIncludeUnmonitoredVideosHelpText')}
              onChange={handleInputChange}
            />
          </FormGroup>

          <FormGroup>
            <FormLabel>{translate('PlaylistPremieresOnly')}</FormLabel>

            <FormInputGroup
              type={inputTypes.CHECK}
              name="premieresOnly"
              value={premieresOnly}
              helpText={translate('ICalPlaylistPremieresOnlyHelpText')}
              onChange={handleInputChange}
            />
          </FormGroup>
          ...
          <FormGroup>
            ...
            <FormInputGroup
              type={inputTypes.CHANNEL_TAG}
              name="tags"
              value={tags}
              helpText={translate('ICalTagsChannelHelpText')}
              onChange={handleInputChange}
            />
          </FormGroup>

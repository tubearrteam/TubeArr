...
function Calendar() {
  ...
  const isRefreshingChannel = useSelector(
    createCommandExecutingSelector(commandNames.REFRESH_CHANNEL)
  );
  ...
  const wasRefreshingChannel = usePrevious(isRefreshingChannel);
  ...
  useEffect(() => {
    if (wasRefreshingChannel && !isRefreshingChannel) {
      dispatch(fetchCalendar({ time, view }));
    }
  }, [time, view, isRefreshingChannel, wasRefreshingChannel, dispatch]);

  useEffect(() => {
    if (!previousItems || hasDifferentItems(items, previousItems)) {
      const videoIds = selectUniqueIds<Video, number>(items, 'id');
      const videoFileIds = selectUniqueIds<Video, number>(
        items,
        'videoFileId'
      );

      if (items.length) {
        dispatch(fetchQueueDetails({ videoIds }));
      }

      if (videoFileIds.length) {
        dispatch(fetchVideoFiles({ videoFileIds }));
      }
    }

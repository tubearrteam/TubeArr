...
function createCalendarEventsConnector(date: string) {
  return createSelector(
    ...
    (state: AppState) => state.calendar.options.collapseMultipleVideos,
    (items, collapseMultipleVideos) => {
      ...
      const groupedObject = Object.groupBy(
        filtered,
        (item: CalendarItem) => `${item.channelId}-${item.playlistNumber}`
      );
      ...
      >((acc, [, events]) => {
        ...
        if (events.length === 1) {
          ...
          acc.push({
            ...
            playlistNumber: events[0].playlistNumber,
            videoIds: events.map((event) => event.id),
            events: events.sort(
              (a, b) =>

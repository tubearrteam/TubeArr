...
export const defaultState = {
  options: {
    includeUnknownChannelItems: true
  },
  ...
  paged: {
    ...
    columns: [
      ...
      {
        name: 'videos.airDateUtc',
        label: () => translate('VideoAirDate'),
        isSortable: true,
        isVisible: false
      },
      {
    ...
    filterBuilderProps: [
      {
        ...
        valueType: filterBuilderValueTypes.CHANNEL
      },
      {
        name: 'quality',
        label: () => translate('Quality'),
...
function fetchDataAugmenter(getState, payload, data) {
  data.includeUnknownChannelItems = getState().queue.options.includeUnknownChannelItems;
}
...
export const CLEAR_DOWNLOAD_QUEUE = 'queue/clearDownloadQueue';
export const START_QUEUE_PROCESSING = 'queue/startQueueProcessing';
...
export const clearDownloadQueue = createThunk(CLEAR_DOWNLOAD_QUEUE);
export const startQueueProcessing = createThunk(START_QUEUE_PROCESSING);
...
export const actionHandlers = handleThunks({
  ...
  [CLEAR_DOWNLOAD_QUEUE]: function(getState, payload, dispatch) {
    dispatch(set({ section: paged, isFetching: true }));
    ...
    promise.done(() => {
      ...
      dispatch(set({ section: paged, isFetching: false, isPopulated: true, error: null }));
    });

    promise.fail((xhr) => {
      dispatch(set({ section: paged, isFetching: false, error: xhr }));
    });
  },

  [START_QUEUE_PROCESSING]: function(getState, payload, dispatch) {
    const promise = createAjaxRequest({
      url: '/queue/process',
      method: 'POST'
    }).request;
    ...
    promise.done((data) => {
      ...
      if (data && data.reason && payload && typeof payload.onError === 'function') {
        payload.onError(data.reason);
      }
    });

    promise.fail((xhr) => {
      const reason = xhr && xhr.responseJSON && xhr.responseJSON.reason ? xhr.responseJSON.reason : (xhr && xhr.status === 503 ? translate('QueueStartFailedYtDlpNotConfigured') : null);
      if (reason && payload && typeof payload.onError === 'function') {
        payload.onError(reason);
      }
    });
  }
});

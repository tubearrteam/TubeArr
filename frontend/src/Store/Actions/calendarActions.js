...
export const defaultState = {
  ...
  options: {
    collapseMultipleVideos: false,
    showVideoInformation: true,
...
function buildCalendarItems(videos, channels, start, end, includeUnmonitored, tagIds) {
  ...
  return videos
    .filter((video) => {
      ...
      const airDateUtc = moment(video.airDateUtc);

      if (!airDateUtc.isValid() || !airDateUtc.isBetween(start, end, undefined, '[]')) {
        return false;
      }

      if (!includeUnmonitored && !video.monitored) {
        return false;
      }

      if (tagIds.length) {
        const channelTags = channelById[video.channelId]?.tags ?? [];

        if (!tagIds.some((tagId) => channelTags.includes(tagId))) {
          return false;
        }
      }
    ...
    .sort((left, right) => moment(left.airDateUtc).valueOf() - moment(right.airDateUtc).valueOf());
}
...
export const actionHandlers = handleThunks({
  [FETCH_CALENDAR]: function(getState, payload, dispatch) {
    ...
    let includeUnmonitored = false;
    let tagIds = [];

    selectedFilters.forEach((selectedFilter) => {
      if (selectedFilter.key === 'unmonitored') {
        includeUnmonitored = selectedFilter.value.includes(true);
      }

      if (selectedFilter.key === 'tags') {
        tagIds = selectedFilter.value;
      }
    });

    const videosRequest = createAjaxRequest({
      url: '/videos'
    }).request;
    ...
    const handleSuccess = (videos, channels = []) => {
    ...
    videosRequest.done((videos) => {
      if (!tagIds.length) {
        handleSuccess(videos);
        return;
      }

      const channelsRequest = createAjaxRequest({
        url: '/channels'
      }).request;

      channelsRequest.done((channels) => {
        handleSuccess(videos, channels);
      });

      channelsRequest.fail(handleFailure);
    });

    videosRequest.fail(handleFailure);
  },

...
export const reducers = createHandleActions({
  ...
  [UPDATE_PATHS]: (state, { payload }) => {
    ...
    const safe = (arr) => (Array.isArray(arr) ? arr : []).filter((item) => item != null && typeof item.path === 'string');

    newState.currentPath = payload.path;
    newState.directories = safe(payload.directories);
    newState.files = safe(payload.files);
